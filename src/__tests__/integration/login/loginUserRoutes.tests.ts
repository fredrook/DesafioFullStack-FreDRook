...
describe("Testing /LOGIN/USERS routes", () => {
  ...
  test("POST /login -  Should not be able to login with  user with isActive = false", async () => {
    ...
    const response = await request(app).post("/login/users").send(mockedInactiveUserLogin)
    await request(app).delete(`/users/${res.body.id}`).set("Authorization", `Bearer ${response.body.token}`)

    const answer = await request(app).post("/login/users").send(mockedInactiveUserLogin)
