...
import app from "../../../app"
import { mockedUser, mockedAdmin, mockedUserLogin, mockedAdminLogin } from "../../mocks/users"
import { mockedCreateDoctor, mockedCreateSpeciality, mockedLoginDoctor } from "../../mocks/contacts"
...
describe("Testing /SCHEDULES routes", () => {
  ...
  test("POST /schedules - ADMIN - Should be able to create a schedule", async () => {
    ...
    const response = await request(app).post("/schedules").set("Authorization", `Bearer ${adminloginReponse.body.token}`).send(scheduleCreate)
  ...
  test("POST /schedules - Schedule already exists", async () => {
    ...
    const response = await request(app).post("/schedules").set("Authorization", `Bearer ${doctorLogin.body.token}`).send(scheduleCreate)
  ...
  test("PATCH /schedules/edit/:id - DOCTORS -  Should not be able to edit schedule without authentication", async () => {
    ...
    const schedulesToBeEdit = await request(app).get("/schedules/doctors").set("Authorization", `Bearer ${doctorLoginResponse.body.token}`)
  ...
  test("PATCH  /schedules/edit/:id -  DOCTORS - Should not be able to edit schedule with invalid id", async () => {
    const loginResponse = await request(app).post("/login/doctors").send(mockedLoginDoctor)
  ...
  test("PATCH /schedules/:id -  USER - Should not be able to schedule without authentication", async () => {
    const loginUserResponse = await request(app).post("/login/users").send(mockedUserLogin)
    const doctor = await request(app).get("/doctors")

    const schedulesToBeScheduleByUser = await request(app).get(`/doctors/${doctor.body[0].id}/schedules`).set("Authorization", `Bearer ${loginUserResponse.body.token}`)
  ...
  test("PATCH  /schedules/:id -  USER - Should not be able to schedule with invalid id", async () => {
    ...
    const response = await request(app).patch(`/schedules/13970660-5dbe-423a-9a9d-5c23b37943cf`).set("Authorization", `Bearer ${loginUserResponse.body.token}`)
  ...
  test("PATCH  /schedules/:id - USER -  Must be able to schedule ", async () => {
    ...
    const scheduleToBeEdit = await request(app).get(`/doctors/${doctor.body[0].id}/schedules`).set("Authorization", `Bearer ${loginUserResponse.body.token}`)

    const response = await request(app).patch(`/schedules/${scheduleToBeEdit.body[0].id}`).set("Authorization", `Bearer ${loginUserResponse.body.token}`)
  ...
  test("DELETE /schedules/:id -  USER - Should not be able to cancel a schedule without authentication", async () => {
    ...
    const schedulesToBeCancelByUser = await request(app).get(`/doctors/${doctor.body[0].id}/schedules`).set("Authorization", `Bearer ${loginUserResponse.body.token}`)
  ...
  test("DELETE  /schedules/:id -  USER - Should not be able to cancel schedule with invalid id", async () => {
    ...
    const response = await request(app).delete(`/schedules/13970660-5dbe-423a-9a9d-5c23b37943cf`).set("Authorization", `Bearer ${loginUserResponse.body.token}`)
    expect(response.status).toBe(404)
    expect(response.body).toHaveProperty("message")
  })
  ...
  test("DELETE  /schedules/:id - USER -  Must be able to cancel schedule ", async () => {
    ...
    const response = await request(app).delete(`/schedules/${schedulesToBeCancelByUser.body[0].id}`).set("Authorization", `Bearer ${loginUserResponse.body.token}`)
    expect(response.body).toHaveProperty("message")
    expect(response.status).toBe(200)
  })
